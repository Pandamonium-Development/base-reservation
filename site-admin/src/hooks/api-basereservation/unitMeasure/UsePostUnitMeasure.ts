...
interface UsePostUnitMeasureProps {
    onSuccess?: (
        data: UnitMeasure,
...
export const UsePostUnitMeasure = ({
    onSuccess,
    onError,
    onSettled
}: UsePostUnitMeasureProps) => {
    const path = '/api/UnitMeasure';
    const method = 'post';

    const postUnitMeasure = UseTypedApiClientBS({ path, method })
    const queryClient = useQueryClient();

    const createUnitMeasureMutation = useMutation({
        mutationKey: ['PostUnitMeasure'],
        mutationFn: async (unitMeasure: UnitMeasureRequest) => {
            const { data } = await postUnitMeasure(castRequestBody(unitMeasure, path, method))
            return data;
        },
    ...
    return createUnitMeasureMutation;
}